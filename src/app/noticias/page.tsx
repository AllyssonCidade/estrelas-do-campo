...
import { CalendarDays, AlertCircle, Loader2 } from 'lucide-react'; // Added AlertCircle, Loader2
...
export default function NoticiasPage() {
  ...
  const fetchNoticias = React.useCallback(async () => {
    ...
    setError(null);
    try {
        console.log("NoticiasPage: Attempting to fetch noticias...");
        const fetchedNoticias = await getNoticiasApi();
        console.log("NoticiasPage: Fetched noticias successfully:", fetchedNoticias.length);
        setNoticias(fetchedNoticias);
    } catch (fetchError: any) {
        const errorMessage = fetchError instanceof Error ? fetchError.message : "Erro desconhecido ao buscar notícias.";
  ...
  return (
    <div className="space-y-6 container mx-auto max-w-4xl py-8 px-4 min-h-[calc(100vh-14rem)]"> {/* Added min-height */}
              ...
              <Card className="shadow-md rounded-lg overflow-hidden flex flex-col sm:flex-row w-full bg-card border hover:shadow-lg transition-shadow duration-150">
                ...
                <div className="relative w-full h-48 sm:w-[100px] sm:h-[100px] flex-shrink-0 bg-muted">
                    <Image
                      ...
                      loading="lazy"
                      // Only unoptimize if using via.placeholder or known non-optimizable sources
                      unoptimized={noticia.imagem?.includes('via.placeholder.com')}
                      onError={(e) => { e.currentTarget.src = "https://via.placeholder.com/100/fecaca/991b1b?text=Erro"; e.currentTarget.alt="Erro"; }}
                    />
                  </div>
